...
public class Robot extends Observable {
	...
	public void sense(boolean realRun) {
        if (realRun) {
            ...
            String[] sensorReadings = sensorData.split(",", sensor.size());
            for (int i = 0; i < sensor.size(); i++) {
                int returnedDistance = (int)Double.parseDouble(sensorReadings[i]);
                ...
                updateMap((returnedDistance), heading, range, x, y, true, sensor.get(i).getAccuracy());
            }
        } else { 
	...
	private void updateMap(int sensedDistance, int direction, int range, int x, int y, boolean realRun, int accuracy) {
		// TODO Auto-generated method stub
	        ...
	        for (int i = 1; i <= distance; i++) {
	            ...
	            else { // if this cell is an obstacle
	            	if (!realRun) {
